...
packages <- c("tidyr", "car", "dplyr", "survival", "ggplot2", "tidyverse",
              ...
              "plotROC", "lmtest", "sandwich") ## Vector of needed packages
...
#Types of treaties
summary(treatydataori$source)
#   Amended by MLI   Amended by protocol    Multilateral        Original    Pre-independence 
##      687                 272                 231                2217                 316 
...
stargazer(desc1, summary=TRUE, 
          ...
          covariate.labels = c("Treaty signing", "Source index", "Pr(Fail)", "BIT signing", "IV: Past BIT signing",
...
stargazer(mltreaty, tobit_2O,    
          ...
          dep.var.labels = c("Treaty signing Source index"),# Is adjusted in Latex
          covariate.labels= c("Pr(Fail)", "IV: Past BIT signing", "Year", "Year squared", "Year cubed",
                              "Bureaucratic quality",
                              "Regime type: Military", 
                              "Regional competition", "Log GDP growth"), 
          digits = 2, 
          omit.stat=c("f"), 
          notes.align = "l", 
...
treatydistribution + My_Theme + xlab("Treaty signing") + 
  ylab("Frequency")
...
probitplot <- ggplot(probitdata, aes(y=predprobit, x=athdatawprob)) +
  geom_point(size = 1, colour = "#FF6666", alpha = 0.7) +
  ylab("Predicted probability of tax treaty signing") + 
  xlab("Pr(Fail)") +
...
ggplot(firstStagePlotValues, 
  ...
  geom_line()+
  ylab("Predicted probability of treaty signing") +
  xlab("Pr(Fail)") + 
...
asymsample$veg <- 'asym'
mainsample$veg <- 'all'
treatylength <- rbind(asymsample, mainsample)
bothLEVELS <- ggplot(treatylength, aes(year, fill = veg)) + 
  geom_histogram(alpha = 0.5, aes(), position = 'identity', bins = 65) + 
  theme_bw() +
...
stargazer(PEheckman, tobit_PEheckman, WHTheckman, tobit_WHTheckman, Otherheckman, tobit_Otherheckman, selection.equation = TRUE, 
          ...
          label = "PEWHT",
          dep.var.labels = c(""),
          column.labels = c("Treaty signing & PE index & Treaty signing & WHT index & Treaty signing & Other index"),# Is adjusted in Latex
          covariate.labels= c("Pr(Fail)", "IV: Past BIT signing", "Year", "Year squared", "Year cubed",
                              "Bureaucratic quality",
                              "Regime type: Military", 
                              "Regional competition", "Log GDP growth"), 
...
stargazer(R4_CIT, tobit_R4_CIT, R4_learning, tobit_R4_learning, R4_capability, tobit_R4_capability, R4_regime, tobit_R4_regime, selection.equation = TRUE, 
          ...
          dep.var.labels = c("Selection: Treaty signing Outcome: Source index"),
...
stargazer(robust2simple, tobit_simple, robust2start, tobit_start, robust2coup, tobit_coup, selection.equation = TRUE, 
          ...
          dep.var.labels = c("Selection: Treaty signing Outcome: Source index"),# Is adjusted in Latex
...
stargazer(probit, Aestimatorlogit, Aestimatorlogitstart,
          ...
          se =list(sqrt(diag(probit$leadercluster)),
                   ...
                   covariate.labels= c("Pr(Fail)", 
                              ...
                              "Bureaucratic Capacity", "Regime type: Military", 
                              "Regional Competition", "Log GDP per capita" ), 
          dep.var.labels = c("Treaty signing"),
          digits = 2, 
          label = "logit",
          omit.stat=c("f"),
